...
CU_signals control_unit(uint32_t insn) {
	...
	// HOWTO?
		...
		signals.set_signals(0,0,3,0,0,1,0); // BEQ
		//fout << "BEQ" << "\n";
...
void Fetch_reg::print_PC() {
	std::cout << "local PC:     0b" << std::bitset<32>(local_PC) << std::endl << std::endl;	
}

Decode_reg::Decode_reg(CU_signals CU_reg_, uint8_t rs1_, uint8_t rs2_, uint32_t rs1_val_, uint32_t rs2_val_, uint8_t rd_, uint16_t imm1_, uint32_t imm2_, uint32_t local_PC_) {
	CU_reg = CU_reg_;
	rs1 = rs1_;
	rs2 = rs2_;
	rs1_val = rs1_val_;
