...
public:
	...
	Fetch_reg(uint32_t val, uint32_t PC_val) { reg = val, local_PC = PC_val; }
	
	uint32_t get_reg() { return reg; }
	uint32_t get_PC() { return local_PC; }
...
public:
	Decode_reg() : rs1(0), rs2(0), rs1_val(0), rs2_val(0), imm1(0), imm2(0), rd(0), local_PC(0) {}
	Decode_reg(CU_signals CU_reg_, uint8_t rs1_, uint8_t rs2_, uint32_t rs1_val_, 
			   uint32_t rs2_val_, uint8_t rd_, uint16_t imm1_, uint32_t imm2_, uint32_t local_PC_);
